...
def sample_show_session(**params):
    planetarium_dome = PlanetariumDome.objects.create(
        name="Milky way", rows=20, seats_in_row=20
    )
...
class AdminAstronomyShowApiTests(TestCase):
    ...
    def test_create_astronomy_show_with_themes(self):
        show_theme1 = ShowTheme.objects.create(name="Action")
        show_theme2 = ShowTheme.objects.create(name="Documental")
        payload = {
            "title": "Mars",
            "show_themes": [show_theme1.id, show_theme2.id],
            "description": "How human can colonization planet?",
        }
        res = self.client.post(ASTRONOMY_SHOW_URL, payload)
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
